// app/api/doi-soat/route.ts -- v2.1
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Tạo bản ghi đối soát vào bảng 9
    ...
    // Cập nhật bảng 7 — tình trạng đối soát
    ...
    if (gh) {
      ...
      if (rowId) {
        await updateRecord(TABLES.GIAO_HANG, Number(rowId), {
          'Tình trạng đối soát': 'Đã đối soát',
          'Trạng thái': ketQua==='Thành công' ? 'Đã giao'
            : ketQua==='Hoàn trả' ? 'Hoàn trả' : 'Đổi hàng',
        })
      }
    }

    // Nếu hoàn thành đơn → cập nhật bảng 5
