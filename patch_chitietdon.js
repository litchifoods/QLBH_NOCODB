...
const new1 = `  // Tính tổng đã thu từ KH
  ...
  // Set mã SP đã có trong chuyến đã đối soát → không cho sửa/hủy
...
// Truyền spDaSoatSet xuống component
...
// 1. Thêm prop spDaSoatSet vào interface
...
const new3 = `  doiSoatMap?: Record<string, any>   // maGH → đối soát
  tongDaThu?: number                  // tổng tiền đã thu từ KH
  spDaSoatSet?: Set<string>           // tên/mã SP đã đối soát → không cho sửa
  user: UserSession`
if (c.includes(old3)) { c = c.replace(old3, new3); console.log('OK 1. prop interface') }
else console.log('FAIL 1. prop interface')

// 2. Thêm spDaSoatSet vào destructuring
...
else console.log('FAIL 2. destructuring')

// 3. Thêm state hủy đơn + modal hủy sau state loading
...
const new5 = `  const [loading,   setLoading]   = useState(false)
  ...
  // Chế độ sửa — chỉ cho sửa khi Chờ giao / Đang giao / Đang giao 1 phần
  const coTheSua = ['Chờ giao','Đang giao','Đang giao 1 phần'].includes(trangThaiTinh||trangThai)
  const [dangSua, setDangSua] = useState(false)`
if (c.includes(old5)) { c = c.replace(old5, new5); console.log('OK 3. state hủy đơn') }
else console.log('FAIL 3. state')

// 4. Thêm hàm hủy đơn sau hàm capNhatTrangThai
const old6 = `  const spHienThi = spList // hiển thị tất cả kể cả đã hủy`
const new6 = `  // ── Hủy toàn bộ đơn hàng ──
  async function xacNhanHuyDon() {
    ...
    setDangHuyDon(true)
    try {
      // Tổng tiền đã thu thực tế từ KH
      const tongDaThuThucTe = tongDaThu || 0
      // Tiền hoàn = tổng đã thu - CP trả hàng
      const tienHoan = Math.max(0, tongDaThuThucTe - cpTraHang)
      const ghiChuHuy = `[Huỷ bởi ${user.hoTen||user.tenDangNhap} lúc ${new Date().toLocaleString('vi-VN')}${lyDoHuy?': '+lyDoHuy:''}]`
...
if (c.includes(old6)) { c = c.replace(old6, new6); console.log('OK 4. hàm hủy đơn') }
else console.log('FAIL 4. hàm')

// 5. Sửa nút "Sửa đơn hàng" — chỉ hiện khi coTheSua + thêm nút Hủy đơn
const old7 = `          {!dangSua && (
...
else console.log('FAIL 5. nút')

// 6. Khóa SP đã đối soát khi sửa — thêm badge 🔒
const old8 = `                        {/* Tên SP */}
                        {dangSua && !sp.da_huy ? (
...
const new8 = `                        {/* Tên SP */}
                        ...
                        {dangSua && !sp.da_huy && !spDaSoatSet.has(sp.tenSP||sp.maSP) ? (
                          <input value={sp.tenSP} onChange={e=>updSP(sp.id,'tenSP',e.target.value)}
                            style={{width:'100%',padding:'3px 6px',border:'1px solid #E5E7EB',borderRadius:'4px',fontSize:'13px',fontWeight:600,marginBottom:'4px'}}/>`
if (c.includes(old8)) { c = c.replace(old8, new8); console.log('OK 6. badge khóa SP') }
...
const new9 = `                      {/* Nút hủy + checkbox */}
                      {dangSua && !sp.da_huy && !spDaSoatSet.has(sp.tenSP||sp.maSP) && (`
if (c.includes(old9)) { c = c.replace(old9, new9); console.log('OK 7. khóa nút hủy SP') }
else console.log('FAIL 7.')

// 8. Thêm modal hủy đơn trước closing tag cuối
const old10 = `      {/* ── Confirm hủy tất cả đã chọn ── */}`
const new10 = `      {/* ── Modal hủy đơn hàng ── */}
      {showHuyDon&&(
        <div className="overlay" onClick={()=>setShowHuyDon(false)}>
          <div className="confirm-box" style={{maxWidth:'440px',textAlign:'left'}} onClick={e=>e.stopPropagation()}>
            ...
            <p style={{fontSize:'13px',color:'#6B7280',textAlign:'center',margin:'0 0 16px'}}>
              <strong style={{color:'var(--primary)'}}>{maDon}</strong> · {(trangThaiTinh||trangThai)}
            </p>
            ...
            <div style={{background:'#F8FAFC',borderRadius:'8px',padding:'12px',marginBottom:'14px',fontSize:'13px'}}>
              ...
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'6px'}}>
                ...
                <input type="number" min="0" value={cpTraHang||''} placeholder="0"
                  ...
                  style={{width:'130px',padding:'4px 8px',border:'1px solid #FCD34D',borderRadius:'4px',fontSize:'13px',textAlign:'right',background:'#FFFBEB'}}/>
              </div>
            ...
            <div style={{background:'#EFF6FF',borderRadius:'6px',padding:'8px 12px',marginBottom:'12px',fontSize:'12px',color:'#1E40AF'}}>
              💡 CP giao hàng của NV/đối tác <strong>giữ nguyên</strong> — công sức đã phát sinh.
            </div>
            ...
            <div style={{marginBottom:'14px'}}>
              ...
              <input className="input" placeholder="VD: Khách đổi ý, hàng bị lỗi..." value={lyDoHuy}
                ...
                style={{fontSize:'13px'}}/>
              {(trangThaiTinh||trangThai)==='Hoàn thành'&&(
                <div style={{fontSize:'11px',color:'#DC2626',marginTop:'3px'}}>Bắt buộc nhập lý do với đơn Hoàn thành</div>
              )}
            </div>
            ...
            <div style={{display:'flex',gap:'10px'}}>
              ...
              <button onClick={()=>setShowHuyDon(false)}
                style={{padding:'11px 16px',borderRadius:'8px',border:'1px solid var(--border)',
...
if (c.includes(old10)) { c = c.replace(old10, new10); console.log('OK 8. modal hủy đơn') }
else console.log('FAIL 8. modal')
