'use client'
// components/InHoaDonClient.tsx — v2.0
// Nâng cấp: logo upload + resize, lưu cài đặt lên NocoDB (server), HÓA ĐƠN BÁN HÀNG căn giữa
...
const MAC_DINH = {
  ...
  logo:       '',      // base64 ảnh logo
  logoSize:   '80',   // kích thước logo tính bằng px
}
...
export default function InHoaDonClient({
  ...
  useEffect(() => {
    fetch('/api/cai-dat')
      ...
      .then(res => {
        if (res.ok && res.data) {
          // Map key từ DB (hoadon_tenCH → tenCH)
          const d = res.data
          setSettings(prev => ({
            ...prev,
      ...
      .catch(() => {
        // Fallback: đọc localStorage nếu server lỗi
        try {
  ...
  const logoSize   = Number(settings.logoSize) || 80
      ...
      <div style={{ padding: '20px' }}>
        <div className="hd-wrapper">
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '16px', marginBottom: '20px' }}>
            ...
            {/* Cột trái: Logo + thông tin cửa hàng */}
            <div style={{ flex: 1 }}>
              {settings.logo && (
                <img
                  src={settings.logo}
              ...
              <div style={{ fontSize: '20px', fontWeight: 800, color: '#1B3A6B', fontFamily: 'serif' }}>
                {settings.tenCH}
              </div>
              {settings.gioiThieu && (
                <div style={{ fontSize: '11.5px', color: '#6B7280', marginTop: '2px', maxWidth: '320px', lineHeight: 1.4 }}>
                  {settings.gioiThieu}
                </div>
              )}
              {settings.diaChiCH && (
                <div style={{ fontSize: '12px', color: '#374151', marginTop: '5px' }}>📍 {settings.diaChiCH}</div>
              )}
              {settings.sdtCH && (
            ...
            {/* Cột phải: Tiêu đề HÓA ĐƠN BÁN HÀNG — căn giữa nổi bật */}
              ...
              {chiTietHopLe.length === 0 ? (
                ...
                <tr key={i}>
                  ...
                  <td>
                    ...
                    {ct['Ghi chú SP'] && (
                      <div style={{ fontSize: '11px', color: '#6B7280', marginTop: '2px' }}>{ct['Ghi chú SP']}</div>
                    )}
                  </td>
...
function ModalCaiDat({ settings, onSave, onClose }: {
  ...
  // Upload logo: convert sang base64
  function handleLogoUpload(e: React.ChangeEvent<HTMLInputElement>) {
    const file = e.target.files?.[0]
    if (!file) return
    // Giới hạn 500KB để tránh quá tải DB
    if (file.size > 500 * 1024) {
      alert('Logo quá lớn! Vui lòng chọn ảnh dưới 500KB.')
      return
    }
    const reader = new FileReader()
    reader.onload = (ev) => {
      const base64 = ev.target?.result as string
      setS(p => ({ ...p, logo: base64 }))
  ...
  return (
    ...
    >
      <div
        style={{ background: 'white', borderRadius: '12px', padding: '24px', width: '100%', maxWidth: '480px', maxHeight: '92vh', overflowY: 'auto' }}
      ...
      >
        ...
        <p style={{ fontSize: '12px', color: '#16A34A', margin: '0 0 16px', background: '#F0FDF4', padding: '8px 12px', borderRadius: '6px' }}>
          ✅ Cài đặt lưu trên <strong>máy chủ</strong> — dùng được trên mọi thiết bị, không mất khi đổi máy.
        </p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              {/* Preview logo */}
              <div style={{
                width: `${Number(s.logoSize) || 80}px`,
                height: `${Number(s.logoSize) || 80}px`,
                ...
                {s.logo
                  ? <img src={s.logo} alt="Logo preview" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
                  : <span style={{ fontSize: '11px', color: '#9CA3AF', textAlign: 'center' }}>Chưa có logo</span>
                }
              </div>
              <div style={{ flex: 1 }}>
                ...
                <button
                  ...
                  style={{ padding: '7px 14px', border: '1px solid #D1D5DB', borderRadius: '6px', background: 'white', cursor: 'pointer', fontSize: '12px', display: 'block', marginBottom: '6px' }}
                >
                  📁 Chọn ảnh logo (tối đa 500KB)
                </button>
                {s.logo && (
                  <button
                    onClick={() => { setS(p => ({ ...p, logo: '' })); setPreviewLogo('') }}
                    style={{ padding: '4px 10px', border: '1px solid #FCA5A5', borderRadius: '6px', background: '#FEF2F2', color: '#DC2626', cursor: 'pointer', fontSize: '11px' }}
                  >
                    🗑️ Xoá logo
                  </button>
                )}
            ...
            <div style={{ marginTop: '10px' }}>
              ...
              <input
                ...
                value={s.logoSize}
                onChange={e => upd('logoSize', e.target.value)}
                style={{ width: '100%' }}
              />
          ...
          {([
            ['Tên cửa hàng *', 'tenCH', 'Nội Thất Tính Tuyết', false],
            ['Địa chỉ cửa hàng', 'diaChiCH', 'Số 123 Đường ABC, TP...', false],
            ['Số điện thoại', 'sdtCH', '0901 234 567', false],
            ['Giới thiệu ngắn', 'gioiThieu', 'Chuyên cung cấp nội thất...', false],
          ] as [string, keyof Settings, string, boolean][]).map(([lb, k, ph]) => (
            <div key={k}>
          ...
          <div>
            ...
            <textarea
              ...
              value={s.mangXH}
              onChange={e => upd('mangXH', e.target.value)}
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              value={s.chanTrang}
              onChange={e => upd('chanTrang', e.target.value)}
            />
          </div>
          ...
          <div style={{ display: 'flex', gap: '10px', marginTop: '4px' }}>
            <button
              onClick={handleSave}
              disabled={saving}
              style={{ flex: 1, padding: '11px', borderRadius: '8px', border: 'none', background: saving ? '#9CA3AF' : '#1B3A6B', color: 'white', fontWeight: 700, fontSize: '14px', cursor: saving ? 'not-allowed' : 'pointer' }}
            >
