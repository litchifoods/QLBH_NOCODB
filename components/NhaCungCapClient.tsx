...
export default function NhaCungCapClient({nccList,ttList,nhapKhoList,user}:{
  nccList:any[]; ttList:any[]; nhapKhoList:any[]; user:UserSession
}) {
    ...
    <div style={{padding:'20px'}}>
        ...
        <div className="card" style={{overflow:'hidden'}}>
          <div style={{overflowX:'auto'}}>
            <table className="ncc-t" style={{width:'100%',borderCollapse:'collapse',fontSize:'13px'}}>
              ...
              <tbody>
                {dsTrang.length===0?(
                  <tr><td colSpan={7} style={{textAlign:'center',padding:'48px',color:'var(--text-muted)'}}>Chưa có nhà cung cấp nào</td></tr>
                  ...
                  return (
                    <tr key={ncc['Id']||i} style={{borderBottom:'1px solid #F0F0F0',background:i%2===0?'white':'#FAFBFD'}}>
                      <td>
                        ...
                        <div style={{fontSize:'11px',color:'#6B7280'}}>{ncc['Mã NCC']}</div>
                      </td>
                      <td style={{fontSize:'12px'}}>
                        {ncc['Số điện thoại']&&<div>📞 {ncc['Số điện thoại']}</div>}
                        {ncc['Địa chỉ']&&<div style={{color:'#6B7280',fontSize:'11px'}}>📍 {ncc['Địa chỉ']}</div>}
                      </td>
                      ...
                      <td style={{textAlign:'right',fontSize:'12px',color:'#16A34A',fontWeight:600}}>{ncc._tongDaTT>0?fVND(ncc._tongDaTT)+'đ':'—'}</td>
      ...
      {view==='detail'&&nccChon&&(<>
        ...
        <div style={{display:'flex',gap:'4px',marginBottom:'14px',borderBottom:'2px solid var(--border)',paddingBottom:'0'}}>
          {([['nhap-kho','📦 Lịch sử nhập kho'],['thanh-toan','💳 Lịch sử thanh toán']] as const).map(([tab,label])=>(
