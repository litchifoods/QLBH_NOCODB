'use client'
// components/GiaoHangClient.tsx — v4.0
// Thêm: cột Tên SP, fix KH+địa chỉ, bỏ CP/Thưởng, click 💰 mở modal đối soát thẳng
...
// ── Kết quả đối soát (dùng chung với modal) ──
const KET_QUA_LIST = [
  ...
  { value:'Đổi hàng — khách trả CP',  label:'🔄 Đổi — khách trả CP',       color:'#1E40AF', bg:'#DBEAFE' },
  { value:'Đổi hàng — cửa hàng chịu', label:'🔄 Đổi — CH chịu CP',         color:'#6D28D9', bg:'#EDE9FE' },
]
...
export default function GiaoHangClient({
  ...
  // ── Modal đối soát nhúng thẳng vào trang Giao hàng ──
  ...
  // Lấy tên SP đầu tiên + tổng số SP của đơn
  function getThongTinSP(maDon: string) {
    const chiTiet = chiTietDonMap[maDon] || []
    const hopLe   = chiTiet.filter((ct:any) => ct['Tên SP (ghi nhanh)'] || ct['Mã SP'])
    if (hopLe.length === 0) return { tenSP: '—', tongSP: 0 }
    const tenDau  = hopLe[0]['Tên SP (ghi nhanh)'] || hopLe[0]['Mã SP'] || '—'
    const tongSP  = hopLe.reduce((s:number, ct:any) => s + Number(ct['Số lượng']||1), 0)
    return { tenSP: tenDau, tongSP, coNhieu: hopLe.length > 1 }
  }
  ...
  function chonDon(don: any) {
    ...
    setDanhSachSP(chiTiet.filter((ct:any) => ct['Tên SP (ghi nhanh)']||ct['Mã SP']).map((ct:any) => {
      ...
      return { maChiTiet:ct['Mã chi tiết']||'', tenSP:ct['Tên SP (ghi nhanh)']||ct['Mã SP']||'—', soLuongDon:sl, daGiao:daDuocGiao, soLuongGiao:conLai, checked:conLai>0, ghiChu:'' }
    }))
  }
  ...
  function chonNguoi(nguoiId: string, nv: any) {
    setDanhSachNguoi(prev => prev.map(n => n.id===nguoiId ? {
      ...
      vaiTroNocoDB:nv['Vai trò']||'',
      hinhThuc:(nv['Mã NV']||'').startsWith('DT-')?'Đối tác':'NV cửa hàng',
  ...
  // ── Mở modal đối soát thẳng từ Giao hàng ──
  function moModalDoiSoat(gh: any) {
  ...
  async function luuDoiSoat() {
    ...
    try {
      const res = await fetch('/api/doi-soat', {
        ...
        body: JSON.stringify({
          maGiaoHang:   modalDS['Mã giao hàng'],
          maChuyen:     modalDS['Mã chuyến']||'',
          maDon:        modalDS['Mã đơn hàng'],
          maNVDoiTac:   modalDS['Mã NV/đối tác']||'',
          tenNVDoiTac:  modalDS['Tên NV/đối tác']||'',
          hinhThucGiao: modalDS['Hình thức giao']||'',
          tienThuKH, hinhThucThu, chiPhiVC, chiPhiLap, thuongChuyen,
          ketQua, ghiChu:ghiChuDS, hoanThanhDon,
        }),
      })
      if (!res.ok) throw new Error((await res.json()).message||'Lỗi')
      setMsg('✅ Đã lưu đối soát thành công'); setMsgOk(true)
    ...
    try {
      const res = await fetch('/api/giao-hang', {
        ...
        body: JSON.stringify({
          ...
          danhSachNguoi: nguoiHopLe.map(n => ({ hinhThuc:n.hinhThuc, maNV:n.maNV, tenNV:n.tenNV, vaiTroChuyen:n.vaiTroChuyen })),
          danhSachSP: spGiao.map(sp => ({ maChiTiet:sp.maChiTiet, tenSP:sp.tenSP, soLuongGiao:sp.soLuongGiao, ghiChu:sp.ghiChu })),
        }),
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data.message||'Lỗi')
      setMsg(`✅ Đã tạo chuyến — ${data.soNguoi} người, ${data.soSP} sản phẩm`); setMsgOk(true)
  ...
  return (
    <div style={{padding:'20px'}}>
      <style>{`
        ...
        .btn-tao{background:var(--primary);color:white;border:none;border-radius:8px;padding:10px 18px;font-size:14px;font-weight:600;white-space:nowrap;cursor:pointer;}
              ...
              {filtered.length===0 ? (
                ...
                return (
                  <tr key={i} style={{borderBottom:'1px solid #F0F0F0',background:i%2===0?'white':'#FAFBFD'}}>
                    <td>
                      <Link href={`/dashboard/don-hang/${maDon}`} style={{color:'var(--primary)',fontWeight:700,textDecoration:'none',whiteSpace:'nowrap'}}>
                        {maDon}
                      </Link>
                    </td>
                    ...
                    {/* ✅ Cột Tên SP — tên SP đầu tiên + tổng số lượng */}
                    <td className="col-sp" style={{maxWidth:'160px'}}>
                      <div style={{fontSize:'12px',fontWeight:600,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}} title={tenSP}>
                        {tenSP}
                      </div>
                      {coNhieu && (
                    ...
                    <td style={{textAlign:'center'}}>
                      <span style={{padding:'3px 9px',borderRadius:'20px',fontSize:'11px',fontWeight:700,
                        ...
                        color:chuaDsRow?'#92400E':'#065F46',whiteSpace:'nowrap'}}>
                        {chuaDsRow ? '⏳ Chưa' : '✅ Đã xong'}
                      </span>
                    </td>
                    <td>
                      {/* ✅ Click mở modal đối soát thẳng — không chuyển trang */}
                      {chuaDsRow ? (
                        <button onClick={()=>moModalDoiSoat(g)}
          ...
          <div className="modal-ds" onClick={e=>e.stopPropagation()}>
            ...
            <div style={{background:'var(--primary-pale)',borderRadius:'8px',padding:'10px 14px',marginBottom:'14px',fontSize:'13px'}}>
              <div style={{fontWeight:700,color:'var(--primary)',marginBottom:'3px'}}>
                ...
                <span style={{marginLeft:'8px',fontSize:'11px',padding:'2px 8px',borderRadius:'10px',background:laDT?'#FEF3C7':'#DBEAFE',color:laDT?'#92400E':'#1E40AF'}}>
              ...
              {Number(donModal?.['Còn phải thu']||0)>0 && (
                <div style={{color:'#DC2626',fontWeight:600,fontSize:'12px',marginTop:'3px'}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
              ...
              <div style={{background:laDT?'#FFF7ED':'#F0F9FF',borderRadius:'8px',padding:'12px 14px',border:`1px solid ${laDT?'#FED7AA':'#BAE6FD'}`}}>
                <div style={{fontWeight:700,fontSize:'13px',marginBottom:'6px',color:laDT?'#C2410C':'#0369A1'}}>
                  {laDT?'💸 Chi phí trả đối tác':'🎁 Thưởng nhân viên'}
                </div>
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'8px'}}>
                  {[['CP vận chuyển','chiPhiVC',chiPhiVC,setChiPhiVC],['CP lắp đặt','chiPhiLap',chiPhiLap,setChiPhiLap],['Thưởng chuyến','thuongChuyen',thuongChuyen,setThuongChuyen]].map(([lb,,val,setter]:any)=>(
                    <div key={lb}>
              ...
              <div>
                ...
                <input className="input" placeholder="Ghi chú thêm..." value={ghiChuDS} onChange={e=>setGhiChuDS(e.target.value)}/>
              </div>
              ...
              <label style={{display:'flex',alignItems:'center',gap:'10px',cursor:'pointer',padding:'10px 12px',background:'#F0FDF4',borderRadius:'8px',border:'1px solid #BBF7D0'}}>
                ...
                <div>
                  ...
                  <div style={{fontSize:'11px',color:'#6B7280'}}>Chỉ tick khi đã giao đủ toàn bộ sản phẩm trong đơn</div>
                </div>
              </label>

              <div style={{display:'flex',gap:'10px'}}>
                <button onClick={luuDoiSoat} disabled={loadingDS}
                  style={{flex:1,padding:'11px',borderRadius:'8px',border:'none',background:loadingDS?'#9CA3AF':'var(--primary)',color:'white',fontWeight:700,fontSize:'14px',cursor:loadingDS?'not-allowed':'pointer'}}>
                ...
                <div>
                  <div style={{position:'relative'}}>
                    <input className="input" placeholder="Gõ mã đơn hoặc tên khách hàng..."
                      ...
                      onFocus={()=>setShowDon(true)} onBlur={()=>setTimeout(()=>setShowDon(false),200)}/>
                    {showDon && (
                      <div className="db">
                        {donLoc.length===0
